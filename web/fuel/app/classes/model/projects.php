...
class Projects extends Base
{
    ...
    public static function lists($mode = null, $limit = null, $open = null, $section_code = null)
    {
        ...
        $datas = \DB::select("*")->from('projects')->where('disable', '=', 0);
        ...
        if ($open === null) {
            ...
            $datas = $datas->where('num_of_open_courses', '>', 0);
        }
    ...
    public static function save($params)
    {
        ...
        \DB::update('projects')->set($params)->where('code', '=', $params['code'])->where('username', '=', $username)->execute();
    ...
    public static function delete($params)
    {
        ...
        $username = \Auth::get('username');
        \DB::update('projects')->set(array('disable' => 1))->where('code', '=', $params['code'])->where('username', '=', $username)->execute();
    ...
    public static function all($section_code = null, $pagination_url, $page, $uri_segment = 3, $per_page = 5)
    {
        ...
        $datas['datas'] = \DB::select('*')->from('projects')
            ->where('status', '=', 1)
            ->where('open_date', '<', \DB::expr('NOW()'));
