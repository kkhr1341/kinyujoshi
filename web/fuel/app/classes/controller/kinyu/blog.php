...
class Controller_Kinyu_Blog extends Controller_Kinyubase
{
    ...
    public function action_search($page = 1)
    {
        ...
        if (Agent::is_mobiledevice()) {
            $this->data['blogs'] = Blogs::all('kinyu' + 'investment', '/report/', $page, 2, 30, Input::get('search_text'));
        } else {
            $this->data['blogs'] = Blogs::all('kinyu' + 'investment', '/report/', $page, 2, 60, Input::get('search_text'));
        }
        $pagination = $this->data['blogs']['pagination'];
